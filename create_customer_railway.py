#!/usr/bin/env python3
"""
สคริปต์สำหรับสร้างลูกค้าตัวอย่างใน Railway
รันใน Railway console หรือ production environment
"""

import os
import sys

def create_customer_railway():
    """สร้างลูกค้าตัวอย่างใน Railway"""
    try:
        # ตั้งค่า environment variables สำหรับ Railway
        if not os.getenv('DATABASE_URL'):
            print("❌ ไม่พบ DATABASE_URL environment variable")
            print("💡 ตรวจสอบว่าอยู่ใน Railway environment หรือไม่")
            return False
        
        # Import หลังจากที่ environment variables ถูกตั้งค่าแล้ว
        from app import app
...
if __name__ == '__main__':
    print("🚀 เริ่มสร้างลูกค้าตัวอย่างใน Railway...")
    success = create_customer_railway()
    if success:
        print("\n✅ เสร็จสิ้น!")
    else:
        print("\n❌ ล้มเหลว!")
